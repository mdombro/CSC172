...
public class Proj2 {
    public static void main(String [] args) throws IOException {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                myQueue post = inFixToPostFix(line);
                Float answer = (float)evalPostFix(post);
                String p = answer.toString();
                fwr.write(dec.format(Float.parseFloat(p))+"\n");
            }
            fwr.flush();
            fwr.close();
            in.close();
    ...
    public static Object evalPostFix(myQueue post) {
      ...
      // +, -, *, /, <, >, =, !, |, &
        ...
        float op1;
        float op2;
        while (post.peek() != null) { //for (int k = 0; k < post.size; k++) {
            Object i = post.dequeue();
            //if (post.peek() != null) i = post.dequeue();
    ...
    public static myQueue inFixToPostFix(String infix) {
            ...
            if (i.getClass() == Character.class) {
                ...
                if (operator((char) i)) {  // if input is an operator +, -, *, /, <, >, =, !, |, &
                    if (s.isEmpty()) {
                        ...
                        q.enqueue(s.pop()); // otherwise pop off stack into queue
        ...
        while (!s.isEmpty()) {
            q.enqueue(s.pop());  // empty any remaining operators onto the output queue
        }
        return q;
    }
